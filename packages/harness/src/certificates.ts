...
/**
 * The trusted test authority of the fake mailbox harness (SPEC section 12).
 *
 * Every server the harness runs presents a leaf this authority signed, and
 * every client the suites drive trusts the authority through `trustedCaPem`.
 * The negative cases stay negative: an expired leaf, a wrong-host leaf, and a
 * self-signed leaf are all exercised with validation enabled, so no test
 * weakens certificate checking. Leaves share one key pair; a certificate is
 * the signed statement about it, so one key serves every case alike.
 */
...
export function createTestAuthority(commonName = "Mail Hub Harness Test Authority"): TestAuthority {
